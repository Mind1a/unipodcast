            ...
            {isOpen && (
                ...
                >
                    <motion.div
                        ...
                        className="bg-[#1E1D2E] max-w-190 w-full text-white p-[20px_24px] rounded-3xl cursor-default relative overflow-hidden flex flex-col"
                    >
                        ...
                        {submitted ? (
                            <div className="z-10 bg-[#1E1D2E] flex flex-col gap-6 items-center justify-center">
                                <h2 className="font-bold text-[54px] leading-100% tracking-[1.51px] font-display"><LgvBebasText>წარმტებით გადაგზავნილია </LgvBebasText></h2>

                                <p className="text-[24px] leading-100% text-[#F4EAFD]">ჩვენი გუნდი 24 საათის განმავლობაში დაგიკავშირდება.</p>

                                <Image alt="Message Read" src={MessageRead} width={483} height={284} />

                                <div className="flex gap-7.5 p-[29px_0px_29px]">
                                    <button
                                        onClick={() => setSubmitted(false)}
                                        className="font-bold text-[14px] leading-100% text-[#FED403] cursor-pointer hover:underline">
                                        {"შეამოწმე ფორმა".toUpperCase()}
                                    </button>

                                    <button
                                        onClick={() => { reset(); setSubmitted(false); }}
                                        className="cursor-pointer p-[14px_16px] border-2 border-[#8A2BE2] rounded-[30px] text-[14px] leading-100% font-bold bg-[#8A2BE233] hover:bg-[#8A2BE260] transition duration-300">
