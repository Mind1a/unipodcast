...
export default function Card({ data }: DataType) {
  return (
    <article className="bg-[#10001F]   flex-col md:flex-row border border-[#28014D] flex w-full  gap-8 p-8 rounded-4xl">
      ...
      <div className="flex flex-col gap-8 w-full  ">
        ...
        <div className="flex flex-col gap-4 items-center sm:items-start  xl:gap-0 xl:flex-row xl:items-center justify-between">
          <div className="flex items-center w-full text-[16px] leading-[100%]  text-[#F4EAFD]">
            <span className="font-medium pr-2 pb-2">სტუმრები:</span>
            <div className="flex items-center  overflow-x-auto  w-full text-nowrap">
              {
                data.guests.map((item, index) => (
                  <span key={index} className="flex pb-2 items-center  gap-2">
                    {item}
          ...
          <div className="flex items-center sm:max-w-72.75  py-1.25 px-9 border text-[#F4EAFD] text-[16px] leading-[100%] gap-5  bg-[#3E2C50] border-[#512A76] rounded-full">
