...
export default function LogoShape() {
  ...
  return (
    <div
      className="relative w-full max-w-[476px] ms-auto"
    ...
    >
      ...
      <div className="relative h-full flex flex-col items-center justify-center px-12 gap-10">
        <div className="overflow-hidden w-full" ref={emblaRef}>
          <div className="flex items-center">
            {logos.map((logo, i) => (
              <div
                key={i}
                className="flex-[0_0_50%] flex items-center justify-center px-4"
              >
                <Image
                  src={logo.src}
                  alt={logo.alt}
          ...
          {scrollSnaps.map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                i === selectedIndex ? "w-6 bg-[#FED403]" : "w-2 bg-[#D9D9D9]"
              }`}
            />
          ))}
        </div>
